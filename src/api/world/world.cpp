...
void world::registry_wf(world_feature *p_world_feature) {
    ...
    api::task::synchronize(p_world_feature);
...
void world::on_update() {
    ...
    if (util::reset_when(this->chunk_checker_timing, api::app.setting.chunk_generation_interval.get_value())) {
        glm::vec3 sub {};
        auto &p_player {api::world::current_player()};
        std::vector<chunk*> current_loaded_chunk {};

        int32_t chunk_size {api::app.setting.chunk_dimension.get_value()};
        int32_t chunk_gen_dist {api::app.setting.chunk_generation_distance.get_value()};
        ...
        for (int32_t z {-chunk_gen_dist}; z != chunk_gen_dist; z++) {
            for (int32_t x {-chunk_gen_dist}; x != chunk_gen_dist; x++) {
                ...
                p_chunk->metadata = {
                        api::app.setting.chunk_fbm_frequency.get_value(),
                        api::app.setting.chunk_fbm_amplitude.get_value(),
                        api::app.setting.chunk_fbm_persistence.get_value(),
                        api::app.setting.chunk_fbm_lacunarity.get_value(),
                        api::app.setting.chunk_fbm_octaves.get_value()
                };
...
void world::on_event_changed_priority(SDL_Event &sdl_event) {
    ...
    if (p_world_feature_target != nullptr && !*p_high && p_world_feature_target->get_priority() != enums::priority::low) {
        ...
        std::vector<world_feature*> new_wf_high_priority_list {};
        for (world_feature *&p_world_feature : this->wf_high_priority_list) {
